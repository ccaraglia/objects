...
var getFullNames = function(array){
    ...
    for (var i=0;i<array.length;i++){
        newArray[i]=array[i].first+ " " + array[i].last
    }
    return newArray
}
...
dog.speak=function(string){
    ...
    for (var i=0; i<array.length;i++){
        array[i]="r"+array[i].substring(1,array[i].length)
    }
    return array.join(" ")
...
//
var pluck = function(listOfObjects, propertyName){
var newArray=[]

    for(var i=0;i<listOfObjects.length;i++){
    newArray[i]=listOfObjects[i][propertyName]
    }
return newArray
}
...
var removePunct = function(string){
    var arrayPunct=[",", "'", ".","-",":","’", "?", "“", "”", "—"]
    var test = string.split("")
    for(var i=0;i<test.length;i++){
            for(var j=0;j<arrayPunct.length;j++){
                if (test[i]===arrayPunct[j]){
                test[i]=""
                }
            }
    }
...
var getCounts=function(string){
    var toCount = removePunct(string).toLowerCase()
    var array=toCount.split(" ")
...
var newArray=[]
    ...
    for(var i=0; i<array.length; i++){
        newArray[i]=reverseObject(array[i])
    }
    return newArray
}
...
politeObject.personalize = function(func){
